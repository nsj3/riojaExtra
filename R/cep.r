...
#  }
  ...
  ret <- .Call("ReadCornellFile", fName, as.double(mValue), as.double(impZero), PACKAGE="riojaExtra")
  if (!is.null(ret[[1]])) {
...
write.CEP <- function(x, fName, fTitle=fName, type = "condensed", nSig=5, nCouplets=4, mValue=-99.9) {
  ...
  ret <- .Call("WriteCornellFile", x, fName, fTitle, as.integer(itype), as.integer(nSig), as.integer(nCouplets), as.integer(nSig), as.integer(nCouplets), as.double(mValue), PACKAGE="riojaExtra")
  if (nchar(ret)>0) {
