...
//   FILE *fout; 
   ...
   if (!fout) {
      delete pData;
      snprintf(str, 1000, "Cannot open file %s", CHAR(STRING_ELT(sexpfName, 0)));
...
//   fprintf(fout, "%5d%5d %-s", nNumericFields, nRecords, NULL); //, CHAR(STRING_ELT(sexpTitle, 0)));
   snprintf(str, 1000, "%5d%5d %-s", nNumericFields, nRecords, CHAR(STRING_ELT(sexpTitle, 0)));
...
//  adjuster for correct field width output
   if (SubFileType == 1) {
      ...
      snprintf(str2, 1000, "(I5,3X,%d(I4,F%d.0))", CornellCnCouplets, CornellCnPlaces+nAdjust+1);
...
//            s.Format("%*.*g", CornellCnPlaces+nAdjust, CornellCnPlaces, x);
            snprintf(str2, 1000, "%*.*g", CornellCnPlaces+nAdjust, CornellCnPlaces, x);
            if (strlen(str2) > (size_t) (CornellCnPlaces+nAdjust))
               nFieldWidthWarnings++;
            snprintf(str, 1000, "%4d %s", j+1, str2);
            fwrite(str, sizeof(char), strlen(str), fout);
         }
         else {
//            s.Format("%*.*g", CornellFnPlaces+nAdjust, CornellFnPlaces, x);
            snprintf(str2, 1000, "%*.*g", CornellFnPlaces+nAdjust, CornellFnPlaces, x);
            if (strlen(str2) > (size_t) CornellFnPlaces+nAdjust)
               nFieldWidthWarnings++;
            snprintf(str, 1000, " %s", str2);
            fwrite(str, sizeof(char), strlen(str), fout);
         }
         nitems++;
      }
   ...
   if (SubFileType == 0) {
      ...
      for (int j=0;j<nNumericFields;j++) {
         ...
         snprintf(str, 1000, " %*.*g", CornellFnPlaces+nAdjust, CornellFnPlaces, x);
         fwrite(str, sizeof(char), strlen(str), fout);
         nitems++;
      }
   }
   ...
   for (j=0;j<nNumericFields;j++) {
      ...
      snprintf(str, 1000, "%8.8s", CHAR(STRING_ELT(VECTOR_ELT(dimnames, 1), j)));
      fwrite(str, sizeof(char), strlen(str), fout);
      nitems++;
   }
   snprintf(str, 1000, "\n");
   ...
   for (j=0;j<nRecords;j++) {
      ...
      snprintf(str, 1000, "%8.8s", CHAR(STRING_ELT(VECTOR_ELT(dimnames, 0), j)));
      fwrite(str, sizeof(char), strlen(str), fout);
      nitems++;
   }
   snprintf(str, 1000, "\n");
